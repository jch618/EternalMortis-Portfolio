...
#include "Types/MortisCombatTypes.h"

void UMortisEnemyCombatComponent::OnHitTargetActor(AActor* HitActor, const FHitResult& HitResult)
{
	Super::OnHitTargetActor(HitActor, HitResult);
	...
	UAbilitySystemComponent* EnemyASC = AbilitySystemInterface->GetAbilitySystemComponent();
	if (!EnemyASC)
	{
		return;
	}
	...
	if (AMortisPlayerCharacter* Player = Cast<AMortisPlayerCharacter>(HitActor))
	{
		...
		if (TargetASC->HasMatchingGameplayTag(MortisGameplayTags::State_Invincible))
		{
			return;
		}
		...
		FGameplayTag DynamicCueTag = DefaultHitCueTag;
		if (CombatVfxData)
		{
			for (const TPair<FGameplayTag, FGameplayTag>& Pair : CombatVfxData->MaterialToHitCueMap)
			{
				if (TargetASC->HasMatchingGameplayTag(Pair.Key))
				{
					DynamicCueTag = Pair.Value;
					break;
				}
			}
		}
		...
		CueParams.Instigator = GetOwningPawn();
		CueParams.EffectContext = EnemyASC->MakeEffectContext();
		CueParams.EffectContext.AddHitResult(HitResult);
		
		TargetASC->ExecuteGameplayCue(DynamicCueTag, CueParams);
	}
	else if (HitActor->GetRootComponent() && HitActor->GetComponentByClass<UStaticMeshComponent>())
	{
		// MORTIS_LOG("Hit Static: %s", *HitActor->GetActorNameOrLabel());
		OnHitEnvironment.Broadcast(HitActor, HitResult, CurrentAttackStyleTag);
	}
...
const FMortisWeaponCommonData* UMortisEnemyCombatComponent::GetUnarmedData() const
{
