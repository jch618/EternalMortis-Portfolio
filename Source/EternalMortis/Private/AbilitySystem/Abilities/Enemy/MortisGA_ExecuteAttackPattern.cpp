	...
	bool bReplicateEndAbility, bool bWasCancelled)
{
	...
	if (UMortisEnemyCombatComponent* CombatComponent = GetEnemyCombatComponent())
	{
		CombatComponent->SetCurrentAttackStyleTag(FGameplayTag::EmptyTag);
	}
...
void UMortisGA_ExecuteAttackPattern::ExecuteNextStep()
{
	...
	if (UMortisEnemyCombatComponent* CombatComponent = GetEnemyCombatComponent())
	{
		CombatComponent->SetCurrentAttackStyleTag(Step.AttackStyleTag);
	}
...
void UMortisGA_ExecuteAttackPattern::OnHitEventReceived(FGameplayEventData Payload)
{
	...
	const FMortisWeaponCommonData* WeaponCommonData = Weapon ? Weapon->GetCommonData() : CombatComponent->GetUnarmedData();
	if (!WeaponCommonData)
	{
		MORTIS_LOG("Weapon Data is null");
		return;
	}
	const FGameplayEffectSpecHandle& SpecHandle = MakeWeaponDamageEffectSpecHandle(*WeaponCommonData);
...
{
	TSubclassOf<UGameplayEffect> EffectClassToUse = AttackPattern->Steps[CurrentStepIndex].OverrideDamageEffectClass ? AttackPattern->Steps[CurrentStepIndex].OverrideDamageEffectClass	: DamageEffectClass;
	FGameplayEffectSpecHandle SpecHandle = MakeDamageEffectSpecHandle(EffectClassToUse, WeaponData, MortisGameplayTags::Data_Enemy_Stat_WeaponDamage);
