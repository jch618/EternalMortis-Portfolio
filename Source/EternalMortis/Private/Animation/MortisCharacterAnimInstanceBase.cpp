...
void UMortisCharacterAnimInstanceBase::NativeThreadSafeUpdateAnimation(float DeltaSeconds)
{
	...
	if (Velocity.SizeSquared2D() > 50.f)
	{
		LocomotionDirection = UKismetAnimationLibrary::CalculateDirection(OwningCharacter->GetVelocity(), OwningCharacter->GetActorRotation());
		FRotator CurrentRot = FRotator(0.f, SmoothedDirection, 0.f);
		FRotator TargetRot = FRotator(0.f, LocomotionDirection, 0.f);
		SmoothedDirection = FMath::RInterpTo(CurrentRot, TargetRot, DeltaSeconds, DirectionInterpSpeed).Yaw;
	}
}
