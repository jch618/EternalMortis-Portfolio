	...
	if (APawn* HitPawn = Cast<APawn>(OtherActor))
	{
		if (UMortisFunctionLibrary::IsTargetPawnHostile(WeaponOwningPawn, HitPawn))
		{
			...
			if (!bFromSweep)
			{
				...
				OtherComp->GetClosestPointOnCollision(WeaponLocation, ClosestPointOnEnemy);

				FixedHitResult.ImpactPoint = ClosestPointOnEnemy;
				FixedHitResult.Location = ClosestPointOnEnemy;
                
				FixedHitResult.ImpactNormal = (WeaponLocation - ClosestPointOnEnemy).GetSafeNormal();
			}
			OnWeaponHitTarget.ExecuteIfBound(OtherActor, FixedHitResult);
		}
	}
}
