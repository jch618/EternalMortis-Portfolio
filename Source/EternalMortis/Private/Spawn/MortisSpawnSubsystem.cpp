...
void UMortisSpawnSubsystem::AssignEnemies()
{
	...
	const UMortisEnemySettings* SpawnSettings = GetDefault<UMortisEnemySettings>();
	if (!SpawnSettings)
	{
	...
	if (!SpawnSettings->FloorSpawnPools.IsValidIndex(CurrentFloor))
	{
		MORTIS_LOG("%d is invalid index!", CurrentFloor);
		return;
	}
