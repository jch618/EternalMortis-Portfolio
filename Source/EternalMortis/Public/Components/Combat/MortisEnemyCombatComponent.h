...
public:
	...
	const FMortisWeaponCommonData* GetUnarmedData() const;
	
	FORCEINLINE void SetCurrentAttackStyleTag(const FGameplayTag& AttackStyleTag) { CurrentAttackStyleTag = AttackStyleTag; }
	
protected:
	UPROPERTY()
	TObjectPtr<const UMortisAttackPatternData> AttackPatternData;

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Mortis|Combat|AttackPattern")
...
private:
	...
	UPROPERTY(Transient)
	FGameplayTag CurrentAttackStyleTag = FGameplayTag::EmptyTag;
};
